...
class OpenCV2 is repr('CPointer')
	{
	constant CV_AUTOSTEP       = 0x7fffffff;
	constant CV_NO_DEPTH_CHECK = 1;
	constant CV_NO_CN_CHECK    = 2;
	constant CV_NO_SIZE_CHECK  = 4;

	constant CV_CMP_EQ = 0;
	constant CV_CMP_GT = 1;
	constant CV_CMP_GE = 2;
	constant CV_CMP_LT = 3;
	constant CV_CMP_LE = 4;
	constant CV_CMP_NE = 5;

	constant CV_MAX_ARR = 10;

	# Fast arctangent calculation
	#
	sub cvFastArctan( num64 $y, num64 $x )
		returns num64
		is native(LIB) { * }

	# Fast cubic root calculation
	#
	sub cvCbrt( num64 $value )
		returns num64
		is native(LIB) { * }

	constant CV_CHECK_RANGE = 1;
	constant CV_CHECK_QUIET = 2;

	constant CV_RAND_UNI    = 0;
	constant CV_RAND_NORMAL = 1;

	constant CV_SORT_EVERY_ROW    = 0;
	constant CV_SORT_EVERY_COLUMN = 1;
	constant CV_SORT_ASCENDING    = 0;
	constant CV_SORT_DESCENDING   = 16;

	constant CV_GEMM_A_T = 1;
	constant CV_GEMM_B_T = 2;
	constant CV_GEMM_C_T = 4;

	constant CV_SVD_MODIFY_A = 1;
	constant CV_SVD_U_T      = 2;
	constant CV_SVD_V_T      = 4;

	constant CV_LU       = 0;
	constant CV_SVD      = 1;
	constant CV_SVD_SYM  = 2;
	constant CV_CHOLESKY = 3;
	constant CV_QR       = 4;
	constant CV_NORMAL   = 16;

	constant CV_COVAR_SCRAMBLED = 0;

	constant CV_COVAR_NORMAL    = 1;
	constant CV_COVAR_USE_AVG   = 2;
	constant CV_COVAR_SCALE     = 4;
	constant CV_COVAR_ROWS      = 8;
	constant CV_COVAR_COLS      = 16;

	constant CV_PCA_DATA_AS_ROW = 0;
	constant CV_PCA_DATA_AS_COL = 1;
	...
	constant CV_L1           = 2;
	constant CV_L2           = 4;
	constant CV_NORM_MASK    = 7;
	constant CV_RELATIVE     = 8;
	constant CV_DIFF         = 16;
	constant CV_MINMAX       = 32;

#	constant CV_DIFF_C       (CV_DIFF | CV_C)
#	constant CV_DIFF_L1      (CV_DIFF | CV_L1)
#	constant CV_DIFF_L2      (CV_DIFF | CV_L2)
#	constant CV_RELATIVE_C   (CV_RELATIVE | CV_C)
#	constant CV_RELATIVE_L1  (CV_RELATIVE | CV_L1)
#constant CV_PCA_USE_AVG     = 2;

	constant CV_REDUCE_SUM = 0;
	constant CV_REDUCE_AVG = 1;
	constant CV_REDUCE_MAX = 2;
	constant CV_REDUCE_MIN = 3;

	constant CV_DXT_FORWARD  = 0;
	constant CV_DXT_INVERSE  = 1;
	constant CV_DXT_SCALE    = 2; # divide result by size of array
	constant CV_DXT_INV_SCALE = (CV_DXT_INVERSE + CV_DXT_SCALE);
	constant CV_DXT_INVERSE_SCALE = CV_DXT_INV_SCALE;
	constant CV_DXT_ROWS     = 4; # transform each row individually
	constant CV_DXT_MUL_CONJ = 8; # conjugate the second argument of cvMulSpectrums

	constant CV_FRONT = 1;
	constant CV_BACK  = 0;

	constant CV_GRAPH_VERTEX        = 1;
	constant CV_GRAPH_TREE_EDGE     = 2;
	constant CV_GRAPH_BACK_EDGE     = 4;
	constant CV_GRAPH_FORWARD_EDGE  = 8;
	constant CV_GRAPH_CROSS_EDGE    = 16;
	constant CV_GRAPH_ANY_EDGE      = 30;
	constant CV_GRAPH_NEW_TREE      = 32;
	constant CV_GRAPH_BACKTRACKING  = 64;
	constant CV_GRAPH_OVER          = -1;

	constant CV_GRAPH_ALL_ITEMS    = -1;

	constant CV_GRAPH_ITEM_VISITED_FLAG     = (1 << 30);
	constant CV_GRAPH_SEARCH_TREE_NODE_FLAG = (1 << 29);
	constant CV_GRAPH_FORWARD_EDGE_FLAG     = (1 << 28);

	constant CV_FILLED = -1;
	constant CV_AA     = 16;

	# basic font types
	constant CV_FONT_HERSHEY_SIMPLEX         = 0;
	constant CV_FONT_HERSHEY_PLAIN           = 1;
	constant CV_FONT_HERSHEY_DUPLEX          = 2;
	constant CV_FONT_HERSHEY_COMPLEX         = 3;
	constant CV_FONT_HERSHEY_TRIPLEX         = 4;
	constant CV_FONT_HERSHEY_COMPLEX_SMALL   = 5;
	constant CV_FONT_HERSHEY_SCRIPT_SIMPLEX  = 6;
	constant CV_FONT_HERSHEY_SCRIPT_COMPLEX  = 7;

	# font flags
	constant CV_FONT_ITALIC                 = 16;

	constant CV_FONT_VECTOR0    = CV_FONT_HERSHEY_SIMPLEX;

	constant CV_KMEANS_USE_INITIAL_LABELS    = 1;

	constant CV_CPU_NONE             = 0;
	constant CV_CPU_MMX              = 1;
	constant CV_CPU_SSE              = 2;
	constant CV_CPU_SSE2             = 3;
	constant CV_CPU_SSE3             = 4;
	constant CV_CPU_SSSE3            = 5;
	constant CV_CPU_SSE4_1           = 6;
	constant CV_CPU_SSE4_2           = 7;
	constant CV_CPU_POPCNT           = 8;
	constant CV_CPU_AVX              = 10;
	constant CV_HARDWARE_MAX_FEATURE = 255;

	constant CV_ErrModeLeaf     = 0;   # Print error and exit program
	constant CV_ErrModeParent   = 1;   # Print error and continue
	constant CV_ErrModeSilent   = 2;   # Don't print and continue
	}
